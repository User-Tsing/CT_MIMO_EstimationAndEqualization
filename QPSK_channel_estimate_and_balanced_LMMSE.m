% QPSK信道估计与均衡，LMMSE估计、迫零均衡（可优化）
function [H, handled_signal_ZF, handled_signal_MMSE, judged_symbols_ZF, judged_symbols_MMSE] = QPSK_channel_estimate_and_balanced_LMMSE(rx_signal, N, M, fs, sign_num, pilot_num, frame_num, pilot_symbol, SNR_dB)
    ...
    for i = 1:frame
        ...
        H_LS = pilot_rx \ pilot_signal; % LS最小二乘法信道估计算法，得到信道估计矩阵（？），理想状态1*2/(1*2)
        R_y = (pilot_rx' * pilot_rx) / size(pilot_rx, 2);
        R_xy = (pilot_signal' * pilot_rx) / size(pilot_rx, 2);
        H(:, :, i) = R_x * (R_x + 1 / noise_power * eye(size(R_x))) \ H_LS;
    end
    % 迫零均衡
