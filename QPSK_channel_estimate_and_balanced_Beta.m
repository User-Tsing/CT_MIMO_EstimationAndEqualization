% QPSK信道估计与均衡_Beta，LS估计、迫零均衡（可优化）
function [H, handled_signal_ZF, handled_signal_MMSE, judged_symbols_ZF, judged_symbols_MMSE] = QPSK_channel_estimate_and_balanced_Beta(rx_signal, N, M, fs, sign_num, pilot_num, frame_num, pilot_symbol, SNR_dB)
    ...
    for i = 1:frame
        ...
        % H(:, :, i) = (pilot_signal' * pilot_signal) \ pilot_signal' * pilot_rx; % LS最小二乘法信道估计算法原版，效果离谱原因不明
        % H(:, :, i) = (pilot_signal' * pilot_signal) \ (pilot_signal' * pilot_rx); % LS最小二乘法信道估计算法，得到信道估计矩阵（？），理想状态1*2/(1*2)
    end
    % 迫零均衡
